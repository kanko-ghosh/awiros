...
class HelmetIO:
    ...
    def classify_person_list(self, person_ls):
        ...
        for person in person_ls:
            ...
            res.append("Wearing Helmet" if pred[0][0] < 0.3 else "Not Wearing Helmet ... ")
        return res
    ...
    def classify_mobile_usage(self, person_ls):
        ...
        for person in person_ls:
            ...
            hyp = ((output[:,0]*output[:,1] < 0).int()).cpu().numpy()
            # hyp = [0, 0, 1, 1]
            pred = predicted_class.item()
            print((abs(output[0][0])+abs(output[0][1])))
            # if hyp[0] == 0 and pred == 0 and output[0][0] <= 1.5:
            if abs(output[0][0]) < 1:
                # print("_______ Toggled _______")
                pred = 1 - pred

            res.append("No Mobile usage" if pred == 0 else "Using Mobile ...")
        return res
    ...
    def convert_v_to_v(self, src_path, dest_path, read_fps = 10.0):
        ...
        imageio.mimsave(dest_path, frame2s, duration=1/fps)
